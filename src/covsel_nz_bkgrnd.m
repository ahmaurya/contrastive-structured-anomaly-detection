function [Z, history] = covsel_nz_bkgrnd(D, B, lambda, rho, alpha)
...
for k = 1:MAX_ITER
    ...
    Z = shrinkage(X_hat + U - B, lambda/rho) + B;

    U = U + (X_hat - Z);

    % diagnostics, reporting, termination checks

    history.objval(k)  = objective(S, B, X, Z, lambda);
...
function obj = objective(S, B, X, Z, lambda)
    obj = trace(S*X) - log(det(X)) + lambda*norm(X(:)-B(:), 1);
end
